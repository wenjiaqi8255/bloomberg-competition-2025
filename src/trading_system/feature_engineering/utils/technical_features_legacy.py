"""
Technical Features Calculator LEGACY!!!!!! - Optimized Implementation.
...
class TechnicalIndicatorCalculator_Legacy:
    """
    ...
    def compute_momentum_features(self, data: pd.DataFrame, periods: List[int],
                                 return_methods: List[str] = None) -> pd.DataFrame:
        """Calculate momentum features for given periods and methods."""
        ...
        for period in periods:
            ...
            if "simple" in return_methods:
                ...
                logger.debug(f"DEBUG: momentum_{period}d has {int(momentum_nan.iloc[0])} NaN values ({int(momentum_nan.iloc[0])/len(momentum)*100:.1f}%)")
            ...
            if "log" in return_methods:
                ...
                logger.debug(f"DEBUG: log_return_{period}d has {int(log_returns_nan.iloc[0])} NaN values ({int(log_returns_nan.iloc[0])/len(log_returns)*100:.1f}%)")

            # Exponential returns (weighted momentum)
            if "exponential" in return_methods:
                # Use exponential weighting for momentum calculation
                ...
                exp_momentum_nan = exp_momentum.isnull().sum()
                period_nan_count += exp_momentum_nan
                features[f'exp_momentum_{period}d'] = exp_momentum
                nan_report[f'exp_momentum_{period}d'] = exp_momentum_nan
                logger.debug(f"DEBUG: exp_momentum_{period}d has {int(exp_momentum_nan.iloc[0])} NaN values ({int(exp_momentum_nan.iloc[0])/len(exp_momentum)*100:.1f}%)")
            ...
            if "simple" in return_methods:
                ...
                logger.debug(f"DEBUG: risk_adj_momentum_{period}d has {int(risk_adj_nan.iloc[0])} NaN values ({int(risk_adj_nan.iloc[0])/len(risk_adj_momentum)*100:.1f}%)")

            # Momentum rank (only if simple momentum was calculated)
            if "simple" in return_methods:
                ...
                logger.debug(f"DEBUG: momentum_rank_{period}d has {int(rank_nan.iloc[0])} NaN values ({int(rank_nan.iloc[0])/len(momentum_rank)*100:.1f}%)")
        ...
        logger.debug(f"DEBUG: rsi_14 has {int(rsi_nan.iloc[0])} NaN values ({int(rsi_nan.iloc[0])/len(rsi)*100:.1f}%)")
        ...
        logger.debug(f"DEBUG: stochastic_k has {int(stoch_k_nan.iloc[0])} NaN values ({int(stoch_k_nan.iloc[0])/len(stoch_k)*100:.1f}%)")
        logger.debug(f"DEBUG: stochastic_d has {int(stoch_d_nan.iloc[0])} NaN values ({int(stoch_d_nan.iloc[0])/len(stoch_d)*100:.1f}%)")
        ...
        logger.debug(f"DEBUG: williams_r has {int(wr_nan.iloc[0])} NaN values ({int(wr_nan.iloc[0])/len(williams_r)*100:.1f}%)")
        ...
        logger.debug(f"DEBUG: mfi has {int(mfi_nan.iloc[0])} NaN values ({int(mfi_nan.iloc[0])/len(mfi)*100:.1f}%)")
    ...
    def compute_volatility_features(self, data: pd.DataFrame, windows: List[int],
                                   volatility_methods: List[str] = None) -> pd.DataFrame:
        """Calculate volatility features for given windows and methods."""
        volatility_methods = volatility_methods or ["std"]
        logger.info(f"DEBUG: Computing volatility features for windows {windows} with methods {volatility_methods} on {len(data)} rows")
        features = pd.DataFrame(index=data.index)
        ...
        for window in windows:
            ...
            if "std" in volatility_methods:
                ...
                # Handle both scalar and Series cases for vol_nan
                vol_nan_count = int(vol_nan.iloc[0]) if hasattr(vol_nan, 'iloc') else int(vol_nan)
                logger.debug(f"DEBUG: volatility_{window}d has {vol_nan_count} NaN values ({vol_nan_count/len(vol)*100:.1f}%)")

                # Volatility of volatility (only if std volatility was calculated)
                ...
                logger.debug(f"DEBUG: vol_of_vol_{window}d has {int(vov_nan.iloc[0])} NaN values ({int(vov_nan.iloc[0])/len(vol_of_vol)*100:.1f}%)")

            # Parkinson volatility (using high/low prices)
            if "parkinson" in volatility_methods:
                high = data['High']
                low = data['Low']
                parkinson_vol = np.sqrt(0.361 * (np.log(high / low) ** 2).rolling(window).mean()) * np.sqrt(252)
                parkinson_nan = parkinson_vol.isnull().sum()
                period_nan_count += parkinson_nan
                features[f'parkinson_vol_{window}d'] = parkinson_vol
                nan_report[f'parkinson_vol_{window}d'] = parkinson_nan
                logger.debug(f"DEBUG: parkinson_vol_{window}d has {int(parkinson_nan.iloc[0])} NaN values ({int(parkinson_nan.iloc[0])/len(parkinson_vol)*100:.1f}%)")
            ...
            if "garman_klass" in volatility_methods:
                ...
                gk_vol = np.sqrt(((0.5 * log_hl ** 2) - (2 * np.log(2) - 1) * log_co ** 2).rolling(window).mean()) * np.sqrt(252)
                gk_nan = gk_vol.isnull().sum()
                period_nan_count += gk_nan
                features[f'gk_vol_{window}d'] = gk_vol
                nan_report[f'gk_vol_{window}d'] = gk_nan
                logger.debug(f"DEBUG: gk_vol_{window}d has {int(gk_nan.iloc[0])} NaN values ({int(gk_nan.iloc[0])/len(gk_vol)*100:.1f}%)")

            # Volatility rank (only if std volatility was calculated)
            if "std" in volatility_methods:
                ...
                logger.debug(f"DEBUG: volatility_rank_{window}d has {int(rank_nan.iloc[0])} NaN values ({int(rank_nan.iloc[0])/len(vol_rank)*100:.1f}%)")

            logger.info(f"DEBUG: Window {window} total NaN values: {period_nan_count}")
        # Additional volatility measures for methods that don't use windows
        if "parkinson" in volatility_methods and "parkinson_volatility" not in features.columns:
            # Only compute if not already computed in windows
            ...
            logger.debug(f"DEBUG: parkinson_volatility has {int(parkinson_nan.iloc[0])} NaN values ({int(parkinson_nan.iloc[0])/len(parkinson_vol)*100:.1f}%)")

        if "garman_klass" in volatility_methods and "garman_klass_volatility" not in features.columns:
            # Only compute if not already computed in windows
            ...
            logger.debug(f"DEBUG: garman_klass_volatility has {int(gk_nan.iloc[0])} NaN values ({int(gk_nan.iloc[0])/len(garman_klass_vol)*100:.1f}%)")

        # Range volatility (always computed if there are any volatility methods)
        if volatility_methods:
            ...
            logger.debug(f"DEBUG: range_volatility has {int(range_nan.iloc[0])} NaN values ({int(range_nan.iloc[0])/len(range_vol)*100:.1f}%)")
    ...
    def compute_technical_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Skip debug logging to avoid format issues
        # initial_nan_count = int(data['Close'].isnull().sum())
        # nan_percentage = float(initial_nan_count/len(data)*100) if len(data) > 0 else 0.0
        # logger.info(f"DEBUG: Input prices have {initial_nan_count} NaN values ({nan_percentage:.1f}%)")
        nan_report = {}
    ...
    def _calculate_directional_movement(self, data: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
        ...
        if dm_plus.ndim > 1:
            dm_plus = dm_plus.flatten()
        if dm_minus.ndim > 1:
            dm_minus = dm_minus.flatten()
