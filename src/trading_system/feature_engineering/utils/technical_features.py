...
class TechnicalIndicatorCalculator:
    ...
    def _calculate_market_factor(self, data: pd.DataFrame) -> Optional[float]:
        """Calculate market factor (excess returns) using DataProcessor."""
        try:
            # Use DataProcessor to calculate returns
            returns = DataProcessor.calculate_returns(data[['Close']])
            if returns.empty or len(returns) < 20:
                return None

            # Calculate excess return over risk-free rate
            avg_return = returns.mean().iloc[0] * 252  # Annualize
    ...
    def _calculate_size_factor(self, data: pd.DataFrame) -> Optional[float]:
        """Calculate size factor (SMB proxy using volatility) with DataProcessor."""
        try:
            # Use DataProcessor to calculate returns and volatility
            returns = DataProcessor.calculate_returns(data[['Close']])
            if returns.empty or len(returns) < 20:
                return None

            # Use volatility as proxy for size (smaller stocks typically more volatile)
            volatility = DataProcessor.calculate_volatility(returns, window=min(len(returns), 252))

            if volatility.empty:
                return None

            vol_value = volatility.iloc[-1, 0]  # Get latest volatility
            # Normalize to 0-1 scale (higher volatility = higher SMB score)
            return max(0, min(1, vol_value / 0.4))  # Map 0-40% vol to [0, 1]

        except Exception:
            return None

    def _calculate_value_factor(self, data: pd.DataFrame) -> Optional[float]:
        """Calculate value factor (HML proxy using momentum) with DataProcessor."""
        try:
            # Use DataProcessor to calculate momentum for different periods
            momentum_data = DataProcessor.calculate_momentum(data[['Close']], periods=[21, 252])

            if momentum_data.empty:
                return None

            short_term_col = f'Close_momentum_21d'
            long_term_col = f'Close_momentum_252d'

            if short_term_col not in momentum_data.columns or long_term_col not in momentum_data.columns:
                return None

            short_term_return = momentum_data[short_term_col].iloc[-1]
            long_term_return = momentum_data[long_term_col].iloc[-1]

            # Value factor: long-term momentum relative to short-term
            if long_term_return > 0 and short_term_return < 0:
    ...
    def _calculate_profitability_factor(self, data: pd.DataFrame) -> Optional[float]:
        """Calculate profitability factor (RMW proxy using return consistency) with DataProcessor."""
        try:
            # Use DataProcessor to calculate returns
            returns = DataProcessor.calculate_returns(data[['Close']])
            if returns.empty or len(returns) < 20:
    ...
    def _calculate_investment_factor(self, data: pd.DataFrame) -> Optional[float]:
        """Calculate investment factor (CMA proxy using low volatility) with DataProcessor."""
        try:
            # Use DataProcessor to calculate returns and volatility
            returns = DataProcessor.calculate_returns(data[['Close']])
            if returns.empty or len(returns) < 20:
            ...
            return max(0, 1 - (vol_value / 0.5))  # Map 0-50% vol to [1, 0]
    ...
    def _calculate_factor_exposures(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        if len(returns) >= 20:
            var_rolling = returns.rolling(60).var()
            # Avoid division by zero which creates inf values
            beta = returns.rolling(60).cov(returns) / (var_rolling + 1e-8)
