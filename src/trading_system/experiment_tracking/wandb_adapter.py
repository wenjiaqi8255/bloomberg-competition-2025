    ...
    def log_figure(self, figure: Any, figure_name: str) -> None:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.html', delete=False) as f:
                if hasattr(figure, 'write_html') or hasattr(figure, 'to_html'):  # Plotly figure
                    try:
                        if hasattr(figure, 'to_html'):
                            html_str = figure.to_html(include_plotlyjs='cdn', full_html=True)
                            f.write(html_str)
                        else:
                            # write_html writes directly to a file path
                            figure.write_html(f.name, include_plotlyjs='cdn', full_html=True)
                    except Exception as e:
                        # Fallback to image conversion if HTML export fails
